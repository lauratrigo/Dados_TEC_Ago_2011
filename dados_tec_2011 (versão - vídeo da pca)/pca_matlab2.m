...
% -------------------------
% 2) Anomalia (mantendo NaNs)
% -------------------------
X_anom = matrix_diffTEC - nanmean(matrix_diffTEC,1);

% -------------------------
% 3) PCA usando ALS (trata NaNs automaticamente)
% -------------------------
[coeff, score, latent, tsquared, explained, mu] = pca(X_anom, 'Algorithm','als', 'Centered', true);

% -------------------------
% 4) EOFs (componentes principais)
% -------------------------
eof1 = coeff(:,1);
eof2 = coeff(:,2);
eof3 = coeff(:,3);
