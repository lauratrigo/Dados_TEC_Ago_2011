...
% -------------------------
% Nomes das estações (copie exatamente nesta ordem)
% -------------------------
...
for i = 1:length(files)
    ...
    matrix_list = [matrix_list data];
end

X_anom = matrix_list - nanmean(matrix_list,1);

[coeff, score, latent, tsquared, explained, mu] = pca(X_anom, ...
    'Algorithm','als', 'Centered', true);

eof1 = coeff(:,2);  % <-- isso é o que vamos plotar

% -------------------------
% INTERPOLAÇÃO para grade
% -------------------------
lon_vec = linspace(min(lon)-1, max(lon)+1, 200);
lat_vec = linspace(min(lat)-1, max(lat)+1, 200);
[Lon, Lat] = meshgrid(lon_vec, lat_vec);

Z = griddata(lon, lat, eof1, Lon, Lat, 'natural');
...
% Plotar o contorno do Brasil (se tiver o shapefile)
try
    ...
    warning('Não foi possível carregar o shapefile do Brasil. Apenas plotará os dados.');
end

% Plotar estações
scatter(lon, lat, 30, 'k', 'filled');

% Texto com nome + valor do EOF
for i = 1:length(lon)
    text(lon(i), lat(i), names{i}, 'FontSize', 7, 'Color','k', 'HorizontalAlignment','center');
    text(lon(i), lat(i)-0.5, num2str(eof1(i),'%.2f'), ...
         'FontSize', 7, 'Color','k', 'HorizontalAlignment','center');
end

title('EOF1 (Padrão espacial) - PCA TEC 2011');
xlabel('Longitude');
ylabel('Latitude');
axis tight;
