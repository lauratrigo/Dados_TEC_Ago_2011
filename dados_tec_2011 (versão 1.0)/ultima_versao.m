...
for k = 1:1
    ...
    for j=1:60:length(eof1)
        
        [Lon, Lat, Z] = teste_grid (lon, lat, eof1(:,j));
        

        % Get contour matrix C, NO PLOT PRODUCED
        C{j} = contourc(Z, levels);
        
        % gera contorno COM coordenadas reais
        C = contourc(Lon(1,:), Lat(:,1), Z, levels);

        % corta o contorno no domínio original
        Ccut = cut_contour(C, ...
                           min(lon), max(lon), ...
                           min(lat), max(lat));
       ...
       %[x, y, Z] = contourf(Lon, Lat, Z, 20, 'LineColor','none');
...
return
%[X,Y,Z] = C2xyz_matrix(C_all{1});

hold on
% for k = 1:length(Z)
%       plot3(X(:,k), Y(:,k), Z(k)*ones(size(X(:,k))))
% end
    
[X,Y,Zc] = C2xyz_matrix(C_all{1});
