...
% Descobre tamanho do primeiro arquivo
tmp = load(fullfile(folder_path, files(1).name));
[nRows, nCols] = size(tmp);

matrix_list = NaN(nRows, nCols*length(files)); % pré-aloca memória
for i = 1:length(files)
    data = load(fullfile(folder_path, files(i).name));
    idx = (i-1)*nCols + (1:nCols);
    matrix_list(:, idx) = data;
end

X_anom = matrix_list - nanmean(matrix_list,1);

% Salva no arquivo 'X_anom.mat'
save('X_anom.mat', 'X_anom');


return

[coeff,~,~,~,~,~] = pca(X_anom,'Algorithm','als','Centered',true);
