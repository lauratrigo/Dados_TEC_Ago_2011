...
% GRID REGULAR (Brasil)
% -------------------------
lonv = linspace(min(lon), max(lon), 200);
latv = linspace(min(lat), max(lat), 200);
[LON, LAT] = meshgrid(lonv, latv);

% -------------------------
% COLORMAP E ESCALA FIXA
% -------------------------
eof_all = score(:,1) * coeff(:,1)';
cmap = jet(256);

% escala fixa do modo 1 (no tempo todo)
A = max(abs(eof_all(:)));
zmin = -A;
zmax =  A;
...
% VÍDEO
% -------------------------
video = VideoWriter('EOF1_testechatinho.mp4','MPEG-4');
video.FrameRate = 10;
open(video);
...
for k = 1:1
    ...
    % reconstrói modo k no tempo
    pca_k = score(:,k) * coeff(:,k)';
    eof_k = pca_k';

    % =========================
    % LOOP TEMPORAL
    % =========================
    for j = 1:10   % ou length(eof_k)

        % -------------------------
        % Interpola estações ? grid
        % -------------------------
        Z = griddata(lon, lat, eof_k(:,j), LON, LAT, 'natural');
        Z(isnan(Z)) = 0;
